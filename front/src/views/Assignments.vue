<template>
<v-container class="d-flex flex-col h-100 w-100 projects" fluid>
    ...
    <v-row class="mt-0 ">
        <v-col class="d-flex flex-col pa-1">
    ...
    >
     <!-- <template v-slot:[`item.link`]="{ item }"> -->
            ...
            <template v-slot:[`item.id`]="{ item }">
                {{item.parent.parent.parent.id}}-{{item.parent.parent.id}}-{{item.parent.id}}-{{item.id}}
            </template>
  ...
  components: { FiltersBar, FInputSearch },
    ...
    methods: {   
        ...
        async updateData(){
         ...
         {
             if ( ['assignment__status__in'].includes(key))  {
             let arr=[]             
        ...
        clickRow(e, row)  {
            ...
            let params = { id:row.item.id}
      
            this.set_assign_id(params)
            this.$router.push({path:`/project/${row.item.parent.parent.parent.id}/task/${row.item.parent.parent.id}/work/${row.item.parent.id}`})
            }
    ...
    mounted() {
        ...
        this.setFields({ 
           assignment__status: { module: "mult" },
        ...
        this.getFilters([
            ...
            {assignment__status:{name:"assignment__status", sort:"id", related_name: "name"}}], true).then(async (data) => {console.log(data,"data")})
